...
def parse_source(html: str, scraped_at: str) -> tuple[list[dict], dict]:
    ...
    numbers = [r["articulo"] for r in records]
    duplicates = sorted({n for n in numbers if numbers.count(n) > 1})
    official_sequence = list(range(min(expected), max(expected) + 1))
    official_gaps = sorted(set(official_sequence) - expected)
    ...
    records.sort(key=lambda r: r["articulo"])
    report = {
        "fuente": SOURCE_URL,
        "fecha_scraping": scraped_at,
        "estado_actual": "documento_actualizado" if updated else "no_determinado",
